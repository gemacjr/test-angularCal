let year = []

let monthOne = [];
let monthTwo = [];
let monthThree = []; 
let monthFour = [];
let monthFive = [];
let monthSix = [];
let monthSeven = [];
let monthEight = [];
let monthNine = [];
let monthTen = [];
let monthEleven = [];
let monthTwelve = [];

let date = new Date();
let isSelectedBool = true;
let isPastBool = false;
let isDefaultBool = false;


let day = { date: date, isSelected: isSelectedBool, isPast: isPastBool, isDefaultBool }

let weekOne = [];
let weekTwo = [];
let weekThree = [];
let weekFour = [];
let weekFive = [];
let weekSix = [];

// try this also let weekOne = Array(7).fill({date: date, isSelected: isSelectedBool, isPast: isPastBool, isDefaultBool});
weekOne = Array(7).fill(day);
// first day of month would be the array location


year = [

    monthOne = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthTwo = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthThree = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthFour = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthFive = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthSix = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthSeven = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthEight = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthNine = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthTen = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthEleven = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],
    monthTwelve = [weekOne = Array(7).fill(day), weekTwo = Array(7).fill(day), weekThree = Array(7).fill(day),
                weekFour = Array(7).fill(day), weekFive = Array(7).fill(day), weekSix = Array(7).fill(day)
    ],

];
